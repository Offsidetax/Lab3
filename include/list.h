...
public:
	...
	/* friend Node* split(const List& current) { //Needed for sort (split list in two halfs)
		...
		std::cout << "Split1 start (First half)\n";
		List check1;
		check1.First = current.First;
		check1.print();
		std::cout << "Split1 finish\n";

		std::cout << "Split2 start (Second half)\n";
		List check2;
		check2.First = mid;
		check2.print();
